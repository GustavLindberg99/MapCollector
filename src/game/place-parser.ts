...
import {getTextWidth, multilineFromPoints, refreshUI} from "../utils.js";
...
export default class PlaceParser{
     ...
     */
    async stations(place: Place): Promise<Array<Station>> {
        this.#stations = [];
        const lineStations = [];
     ...
     */
    async lines(place: Place): Promise<Array<Line>> {
        const lines = [];
     ...
     */
    linesAreSimilar(): (line1: Line, line2: Line) => boolean {
        const similarLines = this.#xmlDocument.querySelector(this.#extraQuery("place"))!!.getAttribute("similarLines");
        switch(similarLines){
        case "name":
            return (line1, line2) => line1.constructor === line2.constructor && line1.name === line2.name;
        case "color":
            return (line1, line2) => line1.constructor === line2.constructor && line1.color === line2.color;
        case null:
            return (line1, line2) => line1.constructor === line2.constructor;
        default:
    ...
    #pointNextToAutostation(stationNode: Element, direction: number): [Point, number] {
        ...
        if(adjacentNode.tagName.toLowerCase() === "point"){
            return [this.#pointFromXml(adjacentNode), autostationsBetween];
        }
    ...
    #vertexFromXml(vertexNode: Element): Branch.Vertex | Branch.OneWayLoop | null {
        ...
        case "oneway":
            ...
            const leftbound = this.#vertexFromXml(vertexNode.children[0]) as Branch.Vertex;
            const rightbound = this.#vertexFromXml(vertexNode.children[1]) as Branch.Vertex;
    ...
    #vertexNextToNode(node: Element, direction: number): Branch.Vertex | Branch.OneWayLoop | null {
        let adjacentNode: Element | null = node;
        let adjacentVertex: Branch.Vertex | Branch.OneWayLoop | null = null;
    ...
    #parseStationList(
        ...
        for(let node of lineNode.children){
            ...
            case "point":
            case "oneway":
                const vertex = this.#vertexFromXml(node)!!;
...
function getLabels(offset: number, lineName: string, previousVertex: Branch.Vertex | Branch.OneWayLoop, nextVertex: Branch.Vertex | Branch.OneWayLoop): Array<Point> {
    ...
    if(previousVertex instanceof Branch.OneWayLoop || nextVertex instanceof Branch.OneWayLoop){
        segments = [new Segment(
            (previousVertex instanceof Branch.OneWayLoop ? previousVertex.leftbound : previousVertex).point,
            (nextVertex instanceof Branch.OneWayLoop ? nextVertex.leftbound : nextVertex).point
        ),
        new Segment(
            (previousVertex instanceof Branch.OneWayLoop ? previousVertex.rightbound : previousVertex).point,
            (nextVertex instanceof Branch.OneWayLoop ? nextVertex.rightbound : nextVertex).point
        )];
    }
    else{
    ...
    const points = segments.map((segment: Segment): Point | null => {
