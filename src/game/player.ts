...
import {ajaxDomain, imgToBase64, sendRequest, staticDomain, wait} from "../utils.js";
...
export default class Player{
     ...
     */
    async goToStation(lineToTake: Line, destination: Station, lookForLast: boolean, fareEvade: boolean): Promise<void> {
    ...
    getCaughtFareEvading(fine: number): void {
        ...
        if(fine > 0){
            ...
            Toastify({
                text: i18next.t("You got caught fare evading and need to pay a fine of ${{arg}}.", {arg: fine}),
                close: true,
                duration: 5000,
                style: {
     ...
     */
    async endGame(won: boolean): Promise<void> {
        this.stopTimer();
        this.fastingForward = false;
        let dialogText = won ? i18next.t("Congratulations! You won!") : i18next.t("Unfortunately you lost. Better luck next time!");
        ...
        if(won){
            ...
            if(isLoggedIn){
                ...
                dialogText += i18next.t("You are not logged in, so your statistics haven't been saved.<br/><a href=\"{{arg}}\">Create a free account</a> if you want your statistics to be saved next time.", {arg: ajaxDomain() + "/users/signup.php"});
            }
        }
        ...
        xdialog.open({
            title: won ? i18next.t("You won") : i18next.t("You lost"),
        ...
        const statisticsToSave: {[key: string]: string} = {
            "won": won.toString(),
            "level": this.#place.level.id(),
            "numberOfMaps": this.maps.size.toString(),
            "remainingTime": (!this.#place.playingWithMoney && this.opponent === null ? this.remainingTime : 0).toString(),
            "remainingMoney": (this.#place.playingWithMoney && this.opponent === null ? this.remainingMoney : 0).toString(),
            "opponentId": this.opponent?.userId.toString() ?? "0"
        };
        const response = await sendRequest(ajaxDomain() + "/ajax/save-statistics.php", new URLSearchParams(statisticsToSave));
        if(response === null){
            const failedStatistics: Array<{[key: string]: string}> = JSON.parse(localStorage.getItem("failedStatistics") ?? "[]");
