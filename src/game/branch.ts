...
import {multilineFromPoints} from "../utils.js";
...
class Branch{
    ...
    pointsAtStation(station: Station): Array<Point> {
        return [
            ...this.#vertices.filter(it => it instanceof Branch.Vertex).filter(it => it.station === station).map(it => it.point),
            ...this.#vertices.filter(it => it instanceof Branch.OneWayLoop).filter(it => it.leftbound.station === station).map(it => it.leftbound.point),
            ...this.#vertices.filter(it => it instanceof Branch.OneWayLoop).filter(it => it.rightbound.station === station).map(it => it.rightbound.point)
        ];
    }
    ...
    stationAtPoint(point: Point): Station | null {
        return this.#vertices.filter(it => it instanceof Branch.Vertex).find(it => it.point?.equalTo(point))?.station ??
               this.#vertices.filter(it => it instanceof Branch.OneWayLoop).find(it => it.leftbound?.point.equalTo(point))?.leftbound.station ??
               this.#vertices.filter(it => it instanceof Branch.OneWayLoop).find(it => it.rightbound?.point.equalTo(point))?.rightbound.station ??
               null;
    }
    ...
    segmentsBetweenStations(start: Station, end: Station, lookForLast: boolean): Array<Segment> | null {
        const findFunction = (callback: (it: Branch.Vertex | Branch.OneWayLoop) => void) => lookForLast ? this.#vertices.findLastIndex(callback) : this.#vertices.findIndex(callback);
        const startIndex = findFunction(it => it instanceof Branch.Vertex ? it.station === start : it.leftbound.station === start || it.rightbound.station === start);
        const endIndex = findFunction(it => it instanceof Branch.Vertex ? it.station === end : it.leftbound.station === end || it.rightbound.station === end);
    ...
    segmentIsTGV(segment: Segment): boolean {
        return this.#vertices.some((it, i) =>
            it instanceof Branch.Vertex
            && it.nextSegmentIsTGV
            && this.#vertexTouchesSegment(it, segment)
            && i + 1 < this.#vertices.length
            && this.#vertexTouchesSegment(this.#vertices[i + 1], segment)
        );
    }

    /**
     * Checks if the given vertex is one of the end points of the given segment, or if one of the vertices of the given one way loop is one of the end points of the given segment.
     *
     * @param vertex    The vertex or one way loop.
     * @param segment   The segment.
     *
     * @returns True if the vertex or one of the vertices of the one way loop is one of the end points of the segment.
     */
    #vertexTouchesSegment(vertex: Branch.Vertex | Branch.OneWayLoop, segment: Segment): boolean {
        return this.#vertexEqualsPoint(vertex, segment.start) || this.#vertexEqualsPoint(vertex, segment.end);
    }

    /**
     * Checks if the given vertex is located at the given point, or one of the two vertices of the given one way loop is located at the given point.
     *
     * @param vertex    The vertex or one way loop.
     * @param point     The point.
     *
     * @returns True if the vertex or one of the vertices of the one way loop is located at the point.
     */
    #vertexEqualsPoint(vertex: Branch.Vertex | Branch.OneWayLoop, point: Point): boolean {
        return vertex instanceof Branch.Vertex ? vertex.point.equalTo(point) : vertex.leftbound.point.equalTo(point) || vertex.rightbound.point.equalTo(point);
    }
    ...
    speedFactor(segment: Segment, positionOnSegment: number): number {
        const startIndex = this.#vertices.findIndex(it => this.#vertexEqualsPoint(it, segment.start));
        const startVertex = this.#vertices[startIndex];
        const endIndex = this.#vertices.findIndex(it => this.#vertexEqualsPoint(it, segment.end));
    ...
    directionAtStation(station: Station): number | null {
        ...
        const currentIndex = this.#vertices.findIndex((vertex: Branch.Vertex | Branch.OneWayLoop): boolean => {
            if(!(vertex instanceof Branch.OneWayLoop)){
                return false;
            }
            else if(vertex.leftbound?.station === station){
                direction = -1;
                return true;
            }
        ...
        let previousVertex: Branch.Vertex;
        switch(previousIndex){
        case -1:
            //If previousIndex is -1 then currentIndex is 0, which means that the above findIndex triggered on index 0, which means that this.#vertices[0] is a one way loop.
            previousVertex = (this.#vertices[0] as Branch.OneWayLoop).leftbound;
            break;
        case this.#vertices.length:
            previousVertex = (this.#vertices.at(-1) as Branch.OneWayLoop).rightbound;
            break;
        default:
            previousVertex = this.#vertexAtIndex(previousIndex, direction);
            break;
        }
        ...
        switch(nextIndex){
        case -1:
            nextVertex = (this.#vertices[0] as Branch.OneWayLoop).rightbound;
            break;
        case this.#vertices.length:
            nextVertex = (this.#vertices.at(-1) as Branch.OneWayLoop).leftbound;
            break;
        default:
            nextVertex = this.#vertexAtIndex(nextIndex, direction);
            break;
        }
    ...
    /**
     * Gets the vertex at the given index. Never returns a one way loop, if the index corresponds to a one way loop uses the `direction` parameter to turn it into a vertex.
     *
     * @param index     The index of the vertex.
     * @param direction The direction of the vertex. Only relevant if the index corresponds to a one way loop.
     *
     * @returns The vertex at the given index.
     */
    #vertexAtIndex(index: number, direction: -1 | 1): Branch.Vertex {
        const vertex = this.#vertices[index];
        if(vertex instanceof Branch.OneWayLoop){
            return direction === -1 ? vertex.leftbound : vertex.rightbound;
        }
        else{
            return vertex;
        }
    ...
    multilines(): Array<Multiline> {
        ...
        let previousVertex: Branch.Vertex | Branch.OneWayLoop | null = null;
        for(let vertex of this.#vertices){
            if(vertex instanceof Branch.OneWayLoop){
                if(previousVertex instanceof Branch.Vertex){
                    for(let oneWayVertex of [vertex.leftbound, vertex.rightbound]){
                        result.push([previousVertex.point, oneWayVertex.point]);
                    }
                }
                else if(previousVertex instanceof Branch.OneWayLoop){
