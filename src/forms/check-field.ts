...
export function checkEmail(input: HTMLInputElement | null, showErrors: boolean = true): void {
    checkField(
        input,
        /^[a-z0-9._%+-]+@[a-z0-9._-]+\.[a-z]{2,}$/i.test(input?.value ?? "") || (!input?.required && input?.value.length === 0),
        showErrors
    );
}
    ...
    */
    ...
    if(password.length === 0 && !input.required){
        ...
        if(toolTip !== null){
            ...
            toolTip.textContent = i18next.t("Leave blank to keep your current password.");
        }
    }
        ...
        if(toolTip !== null){
            ...
            toolTip.textContent = i18next.t("Please enter between {{arg}} and {{count}} characters.", {arg: 3, count: 255});
        }
    }
        ...
        if(toolTip !== null){
            ...
            toolTip.textContent = i18next.t("Your password starts or ends with a whitespace. Is this intentional?");
        }
    }
        ...
        if(toolTip !== null){
            toolTip.setAttribute("class", "formToolTip correct");
            toolTip.textContent = i18next.t("Strong password.");
        }
    }
        ...
        if(toolTip !== null){
            ...
            if(password.length < 8){
                toolTip.textContent = i18next.t("Weak password. We recommend entering least 8 characters.");
            }
            else if(!/[^0-9]/.test(password) || uniqueChars.length < 4){
                toolTip.textContent = i18next.t("Weak password. We recommend using more unique characters.");
            }
            else{
                toolTip.textContent = i18next.t("Weak password.");
            }
        }
    }
    else{
        ...
        if(toolTip !== null){
            toolTip.setAttribute("class", "formToolTip badWarning");
            if(isNumberSequence){
                toolTip.textContent = i18next.t("Very weak password. We don't recommend using common sequences of numbers as passwords.");
            }
            else if(isCommonWord){
                toolTip.textContent = i18next.t("Very weak password. This password is too common.");
            }
            else if(isRepeatedChar || uniqueChars.length <= 2){
                toolTip.textContent = i18next.t("Very weak password. We recommend using more unique characters.");
            }
            else if(isEmail){
                toolTip.textContent = i18next.t("Very weak password. We don't recommend using your email address as a password.");
            }
            else if(password.length < 6){
                toolTip.textContent = i18next.t("Very weak password. We recommend entering least 8 characters.");
            }
            else{
                toolTip.textContent = i18next.t("Very weak password.");
            }
        }
    }
}
...
export function checkForStrongPassword(form: HTMLFormElement): boolean {
    for(let field of [...form.getElementsByTagName("input"), ...form.getElementsByTagName("textarea")]){
        if(field.classList.contains("warning") || field.classList.contains("badWarning")){
            return confirm(i18next.t("The password you chose is weak, which means that it will be easy for people to steal your account. We strongly recommend choosing another password.") + "\n\n" + i18next.t("Do you really want to continue?"));
        }
    }
    return true;
}
